...
const Home: NextPage = () => {
  ...
  if (session) {
    return (
      <>
        ...
        <div className='relative bg-lime-100 min-h-screen pt-8'>
          <ChatWindow />
          <img
            src='/realRecycleMaster.png'
            className='absolute -left-48 bottom-1/4 rotate-12 select-none'
          />
        </div>
        <AskHelp />
      </>
  ...
  return (
    <>
      ...
      <div className='absolute font-bold text-5xl text-white z-50 -translate-x-1/2 -translate-y-1/2 right-[10%] top-1/2'>
        <motion.p variants={vars2} initial='hidden' animate='visible'>
          {start.map((letter) => (
            <motion.span variants={vars}>{letter}</motion.span>
          ))}
        </motion.p>
        <motion.button
